...
fun bilInfoVariabler(
  ...
  val motorOgDrivverk =
    bilInfo?.kjoretoydataListe?.get(0)?.godkjenning?.tekniskGodkjenning?.tekniskeData?.motorOgDrivverk
  var hk = 0
  motorOgDrivverk?.motor?.forEach { motor ->
    motor.drivstoff.forEach { drivstoff ->
      // Sjekker om "maksNettoEffekt" er tilgjengelig
      val nettoEffekt = drivstoff.maksNettoEffekt

      // Hvis ikke, bruk "maksEffektPrTime"
      if (nettoEffekt == 0.0) {
        val effektPrTime = drivstoff.maksEffektPrTime
